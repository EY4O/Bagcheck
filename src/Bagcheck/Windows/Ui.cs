...
internal static class Ui
{
    ...
    public static void Icon(ItemCatalog items, uint itemId, bool hq, float size = 22)
    {
        var scaled = new Vector2(size, size) * ImGuiHelpers.GlobalScale;
        if (items.Get(itemId) is { } info &&
            Plugin.TextureProvider.GetFromGameIcon(new GameIconLookup(info.Icon, hq)).GetWrapOrDefault() is { } texture)
